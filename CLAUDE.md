...
- **Datasets (`ds/`, gitignored) are reconstructed from GitHub Release mirrors** via `python tools/download_data.py` (asset map `tools/data_manifest.json`; publisher `tools/publish_datasets.py`, needs `gh`). Releases: `data-mvs-training`→`ds/mvs_training/dtu/`, `data-depths-raw`→`ds/Depths/`, `data-llff`→`ds/llff/nerf_llff_data/`, `data-rs-dtu`→`ds/rs_dtu/` (+`ds/dtu_example.zip`), `data-dtu`→`ds/dtu/`, `data-finetuning`→`runs_fine_tuning/` (repo root). Archives are zip-store (most rooted at `ds/`; per-asset `extract_to` in the manifest); big datasets are scene-bin-packed into ≤1.95 GiB parts (never byte-split).
- **Depth path gotcha:** the `dtu` (generalization) and `dtu_ft` loaders read images from `Rectified/scanN_train/` but depths from `Depths/scanN/` (un-suffixed) — `data/dtu.py:153-156`, `data/dtu_ft.py:160`. `mvs_training` ships only `scanN_train` depths; plain `Depths/scanN/` come from the `depths-raw` mirror, so those modes need **both** datasets downloaded. Missing depths are silently zero-filled (`data/dtu.py:182-183`).
